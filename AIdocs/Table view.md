...
Add a summary
    ...
    Select lucide-calculator.svg > icon Summarize….
    Choose one of the built-in summary functions, or select lucide-square-function.svg > icon Add summary to define your own.
...
You can define your own summary using a formula:
    ...
    In the lucide-calculator.svg > icon Summarize… menu, choose lucide-square-function.svg > icon Add summary.
    Give the summary a name.
    Enter a formula. The formula runs over the list of values in that column (for example, using a function like values.reduce(...)).
    Save the summary.
