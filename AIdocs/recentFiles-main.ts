...
import { getIconicIconInfo } from './iconic';
...
interface RecentFilesData {
  ...
  maxLength?: number;
  pinnedFiles: string[]; // ピン留めされたファイルのパス配列（表示順序）
  showIconicIcons: boolean;
}
...
class RecentFilesListView extends ItemView {
  ...
  public readonly redraw = (): void => {
    ...
    this.data.recentFiles.forEach((currentFile) => {
      const isPinned =
        this.data.pinnedFiles?.includes(currentFile.path) || false;
      ...
      const navFileTitleIcon = navFileTitle.createDiv({
        cls: 'recent-files-icon',
      });
      ...
      this.renderIconicIcon(navFileTitleIcon, currentFile.path, tFile);
      ...
      navFileTitle.addEventListener('contextmenu', (event: MouseEvent) => {
        ...
        if (isPinned) {
          menu.addItem((item) =>
            item
              .setSection('action')
              .setTitle('Unpin this item')
              .setIcon('lucide-pin-off')
              .onClick(async () => {
                await this.unpinFile(currentFile);
                this.redraw();
              }),
          );
        } else {
          menu.addItem((item) =>
            item
              .setSection('action')
              .setTitle('Pin this item')
              .setIcon('lucide-pin')
              .onClick(async () => {
                await this.pinFile(currentFile);
                this.redraw();
              }),
          );
        }
        ...
        menu.addItem((item) =>
          item
            ...
            .onClick(async () => {
              await this.moveItemUp(currentFile);
              this.redraw();
            }),
        );
        ...
        menu.addItem((item) =>
          item
            ...
            .onClick(async () => {
              await this.moveItemDown(currentFile);
              this.redraw();
            }),
        );
      ...
      // ピンアイコンを追加（削除アイコンの右、最右端に配置）
      // ピン留め済みの場合は常時表示、未ピン留めの場合はホバー時のみ表示
      const navFilePin = navFileTitle.createDiv({
        cls: isPinned
          ? 'recent-files-file-pin menu-item-icon'
          : 'recent-files-file-pin menu-item-icon recent-files-file-pin-hover',
      });
      setIcon(navFilePin, isPinned ? 'lucide-pin' : 'lucide-pin');
      setTooltip(navFilePin, isPinned ? 'Unpin this file' : 'Pin this file');
      navFilePin.addEventListener('click', async (event) => {
        event.stopPropagation();
  ...
  private readonly renderIconicIcon = (
    iconEl: HTMLElement,
    filePath: string,
    tFile: TFile | null,
  ): void => {
    if (!this.plugin.data.showIconicIcons) {
      iconEl.toggleClass('recent-files-icon-hidden', true);
      return;
    }

    const iconInfo = getIconicIconInfo(this.app, filePath);
    if (!iconInfo) {
      iconEl.toggleClass('recent-files-icon-hidden', true);
      return;
    }

    let iconId = iconInfo.icon;
    if (!iconId && iconInfo.color) {
      iconId = this.getDefaultIconId(tFile);
    }
    ...
    iconEl.toggleClass('recent-files-icon-hidden', false);
    iconEl.toggleClass('recent-files-icon-emoji', !this.isLucideIconId(iconId));
    iconEl.empty();

    if (this.isLucideIconId(iconId)) {
  ...
  private readonly isLucideIconId = (iconId: string): boolean =>
    iconId.startsWith('lucide-');

  private readonly getDefaultIconId = (tFile: TFile | null): string => {
    const extension = tFile?.extension ?? '';
    if (extension === 'canvas') {
  ...
  /**
   * ファイルをピン留めする
   * 新しくピン留めされたファイルは配列の先頭（リストの一番上）に追加される
   */
  private readonly pinFile = async (file: FilePath): Promise<void> => {
    if (!this.data.pinnedFiles) {
      this.data.pinnedFiles = [];
    }

    // 既にピン留めされている場合は何もしない
    if (this.data.pinnedFiles.includes(file.path)) {
      return;
    }

    // 配列の先頭に追加（リストの一番上に表示される）
    this.data.pinnedFiles.unshift(file.path);

    // recentFilesの順序を整理
    this.plugin.sortRecentFilesByPinStatus();
   ...
   * ファイルのピン留めを解除する
   */
  private readonly unpinFile = async (file: FilePath): Promise<void> => {
    if (!this.data.pinnedFiles) {
      return;
    }

    this.data.pinnedFiles = this.data.pinnedFiles.filter(
      (path) => path !== file.path,
    );
  ...
  /**
   * アイテムを上に移動する
   * ピン留めアイテムと通常アイテムの境界を越える移動は無効
   */
  private readonly moveItemUp = async (file: FilePath): Promise<void> => {
    const isPinned = this.data.pinnedFiles?.includes(file.path);

    if (isPinned) {
      // ピン留めファイルの場合、pinnedFiles配列内で上に移動
      const index = this.data.pinnedFiles.indexOf(file.path);
      if (index > 0) {
        // 既に先頭でない場合のみ移動
        this.data.pinnedFiles.splice(index, 1);
        this.data.pinnedFiles.splice(index - 1, 0, file.path);
        this.plugin.sortRecentFilesByPinStatus();
        await this.plugin.saveData();
      }
    } else {
      // 通常ファイルの場合、recentFiles配列内で上に移動
      // ただしピン留めゾーンには入らない
      const pinnedCount = this.data.pinnedFiles?.length || 0;
      const index = this.data.recentFiles.findIndex(
        (f) => f.path === file.path,
      );

      if (index > pinnedCount) {
        // ピン留めゾーンより下にあり、かつ境界でない場合のみ移動
        const temp = this.data.recentFiles[index];
        this.data.recentFiles[index] = this.data.recentFiles[index - 1];
        this.data.recentFiles[index - 1] = temp;
        await this.plugin.saveData();
      }
    }
  };
   ...
   * ピン留めアイテムと通常アイテムの境界を越える移動は無効
   */
  private readonly moveItemDown = async (file: FilePath): Promise<void> => {
    const isPinned = this.data.pinnedFiles?.includes(file.path);

    if (isPinned) {
      // ピン留めファイルの場合、pinnedFiles配列内で下に移動
      const index = this.data.pinnedFiles.indexOf(file.path);
      if (index >= 0 && index < this.data.pinnedFiles.length - 1) {
        // 最後尾でない場合のみ移動
        this.data.pinnedFiles.splice(index, 1);
        this.data.pinnedFiles.splice(index + 1, 0, file.path);
        this.plugin.sortRecentFilesByPinStatus();
      ...
      // 通常ファイルの場合、recentFiles配列内で下に移動
      const index = this.data.recentFiles.findIndex(
        (f) => f.path === file.path,
      );

      if (index >= 0 && index < this.data.recentFiles.length - 1) {
        // 最後尾でない場合のみ移動
        const temp = this.data.recentFiles[index];
        this.data.recentFiles[index] = this.data.recentFiles[index + 1];
        this.data.recentFiles[index + 1] = temp;
...
export default class RecentFilesPlugin extends Plugin {
  ...
  public async onload(): Promise<void> {
    ...
    // プラグイン初期化時にピン留め情報に基づいて順序を整理
    this.sortRecentFilesByPinStatus();
  ...
  /**
   * data.recentFilesの順序をピン留め情報に基づいて整理する
   * ピン留めファイルを先頭に、その後に通常ファイルを配置
   */
  public readonly sortRecentFilesByPinStatus = (): void => {
    if (!this.data.pinnedFiles || this.data.pinnedFiles.length === 0) {
      // ピン留めファイルがない場合は何もしない
      return;
    }

    const pinnedFiles: FilePath[] = [];
    const unpinnedFiles: FilePath[] = [];

    // recentFilesをピン留めと非ピン留めに分類
    ...
    // ピン留めファイルをpinnedFiles配列の順序に並べ替え
    const orderedPinnedFiles: FilePath[] = [];
    this.data.pinnedFiles.forEach((path) => {
      const file = pinnedFiles.find((f) => f.path === path);
      if (file) {
        orderedPinnedFiles.push(file);
      }
    });

    // 最終的な順序：ピン留めファイル → 通常ファイル
    this.data.recentFiles = [...orderedPinnedFiles, ...unpinnedFiles];
  };
  ...
  private readonly updateData = async (file: TFile): Promise<void> => {
    const lengthBefore = this.data.recentFiles.length;
    const isPinned = this.data.pinnedFiles?.includes(file.path) || false;
    ...
    if (this.shouldAddFile(file)) {
      ...
      // ピン留め情報に基づいて順序を整理
      // これにより、ピン留めファイルは上部に固定され、
      // 通常ファイルはピン留めファイルの後ろの先頭に配置される
      this.sortRecentFilesByPinStatus();
    }
  ...
  private readonly handleRename = async (
    ...
    if (entry) {
      ...
      // ピン留めリストも更新
      if (this.data.pinnedFiles) {
        const pinnedIndex = this.data.pinnedFiles.indexOf(oldPath);
        if (pinnedIndex !== -1) {
          this.data.pinnedFiles[pinnedIndex] = file.path;
        }
      }
  ...
  private readonly handleDelete = async (
    ...
    if (this.data.pinnedFiles) {
      const beforePinnedLen = this.data.pinnedFiles.length;
      this.data.pinnedFiles = this.data.pinnedFiles.filter(
        (path) => path !== file.path,
      );
      pinnedChanged = beforePinnedLen !== this.data.pinnedFiles.length;
    }


    
    if (beforeLen !== this.data.recentFiles.length || pinnedChanged) {
      this.view?.redraw();
      await this.saveData();
    }
  };
...
class RecentFilesSettingTab extends PluginSettingTab {
  ...
  public display(): void {
    ...
    new Setting(containerEl)
      .setName('Show Iconic icons')
      .setDesc('Add Iconic icons to the recent files list.')
      .addToggle((toggle) => {
        toggle
          .setValue(this.plugin.data.showIconicIcons)
          .onChange((value) => {
            this.plugin.data.showIconicIcons = value;
